{ config, ... }:
{
  users.users.root.openssh.authorizedKeys.keys =
    config.krebs.users."0x4A6F".pubkeys
    ++ config.krebs.users.susanne.pubkeys
    ++ config.krebs.users.hase.pubkeys
    ++ config.krebs.users.neos.pubkeys
    ++ config.krebs.users.raute.pubkeys
    ++ config.krebs.users.ulrich.pubkeys
    ++ config.krebs.users.xq.pubkeys
    ++ config.krebs.users.xkey.pubkeys
    ++ [
    "ssh-rsa  root@plattenschwein" # for backup
    "ssh-rsa  gitlab-builder" # for being deployed by gitlab ci
  ];
}

